...
private var mViewModel = KshAlbumViewModel()
...
@Composable
fun KshAlbumScreen(
    viewModel: KshAlbumViewModel = viewModel(),
) {
...
@Composable
private fun ImageListRowView() {
    LazyRow(
//        modifier = Modifier.background(Color.Yellow).padding(0.dp),
        horizontalArrangement = Arrangement.spacedBy(10.dp),
    ) {
        itemsIndexed(mViewModel.projectList.value) { index, item ->
            if (index != 0) {
                ImageListRowItem(
                    index,
                    item
